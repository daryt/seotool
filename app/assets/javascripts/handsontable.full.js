  ...
  var onMouseUp = function(event) {
    if (event.button !== 2) {
      ...
      if (cell.TD === dblClickOrigin[0] && cell.TD === dblClickOrigin[1]) {
        ...
        // console.log(cell);
        var ngModel = 'db.items[' + cell.coords.col + '][' + cell.coords.row + ']';
        var html = '<input type="text" data-ng-model="' + ngModel + '" style="width: 300px">';
        // console.log(html);
        $('#formCell').html(html);

        angular.element('.main-wrapper').scope().edit(cell.coords.col,cell.coords.row);
        angular.element('.main-wrapper').scope().$apply();
...
Handsontable.Core = function Core(rootElement, userSettings) {
          ...
          for (r = 0; r < rlen; r++) {
            ...
            for (c = 0; c < clen; c++) {
              ...
              if (!instance.getCellMeta(current.row, current.col).readOnly) {
                ...
                if (source === 'paste' && value[0] === '=') {
                  if (current.row != instance.getCopiedCell()[0])
                  {
                    for (var i = 0;i<value.length;i++)
                    {
                      if (isLetter(value[i]) && value[i+1] != '$' && value[i-1] != '$')
                      {
                        var rowVal = '';
                        var j = 1;
                        while(!isNaN(value[i+j]) && j < 10)
                        {
                          rowVal += value[i+j];
                          j++;
                        }
                        rowVal = (parseInt(rowVal) + current.row - instance.getCopiedCell()[0]).toString();
                        value = setStringAt(value,i + 1,rowVal,instance.getCopiedCell()[0].toString().length);
                      }
                    }
                  }

                  if (current.col != instance.getCopiedCell()[1])
                  {
                    for (var i = 0;i<value.length;i++)
                    {
                      if (isLetter(value[i]) && value[i-1] != '$')
                      {
                        // console.log(value.charCodeAt(i));
                        var newCharCode = Math.max(value.charCodeAt(i) + current.col - instance.getCopiedCell()[1], 65);
                        value = setCharAt(value,i,String.fromCharCode(newCharCode));
                      }
                    }
                  }
                }
  ...
  this.selection = selection = {
    ...
    transformStart: function(rowDelta, colDelta, force, keepEditorOpened) {
      ...
      // console.log(cell);
      var ngModel = 'db.items[' + coords.col + '][' + coords.row + ']';
      var html = '<input type="text" data-ng-model="' + ngModel + '" style="width: 300px">';
      // console.log(html);
      $('#formCell').html(html);

      angular.element('.main-wrapper').scope().edit(coords.col,coords.row);
      angular.element('.main-wrapper').scope().$apply();
  ...
  this.getCopiedCell = function() {
    return priv.copiedCell;
  }
  this.setCopiedCell = function(info) {
    priv.copiedCell = info;
  }
...
function CopyPastePlugin(instance) {
  ...
  function onPaste(str) {
    ...
    // console.log(instance.getCopiedCell());


    // inputArray[0][0] = 'pasted';
...
function init() {
  var instance = this,
      ...
      //super hacky way to get the cell copied info
  document.addEventListener('copy', function(e) {
    instance.setCopiedCell(instance.getSelected());
    // this.copiedSelection = instance.getSelected();
